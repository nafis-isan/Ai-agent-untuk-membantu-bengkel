...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-7" }, _attrs))}><section class="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p class="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-[#0f7771]">Senin, 21 September 2026 / Shift pagi</p><h2 class="text-4xl font-bold text-[#172126] md:text-5xl">Meja kerja bengkel.</h2><p class="mt-2 max-w-xl text-sm text-[#6e7a7d]">Satu pandangan untuk antrean, kendaraan, dan suku cadang yang perlu dibereskan hari ini.</p></div>`);
			_push(ssrRenderComponent(_component_NuxtLink, {
				to: "/services",
				class: "inline-flex items-center justify-center gap-2 rounded-md bg-[#d96b35] px-4 py-3 text-sm font-bold text-white shadow-[4px_4px_0_#172126] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[2px_2px_0_#172126]"
			}, {
			...
			ssrRenderList(unref(statCards), (stat) => {
				_push(`<div class="border border-[#d9d8d1] bg-[#fbfaf6] p-5 transition hover:-translate-y-0.5 hover:border-[#0f7771] hover:shadow-[4px_4px_0_#d9d8d1]"><div class="flex items-start justify-between"><div class="${ssrRenderClass([stat.iconBg, "flex h-10 w-10 items-center justify-center rounded-md"])}">`);
				_push(ssrRenderComponent(_component_Icon, {
					name: stat.icon,
					class: ["h-5 w-5", stat.iconColor]
				}, null, _parent));
				_push(`</div><span class="text-[10px] font-bold uppercase tracking-[0.14em] text-[#0f7771]">Live</span></div><p class="mt-5 text-xs font-bold uppercase tracking-[0.12em] text-[#6e7a7d]">${ssrInterpolate(stat.label)}</p><p class="mt-1 text-4xl font-bold text-[#172126]">${ssrInterpolate(stat.value)}</p><p class="mt-2 text-xs text-[#8a9694]">${ssrInterpolate(stat.note)}</p></div>`);
			});
			_push(`<!--]--></section><section class="border-l-4 border-[#e9b949] bg-[#172126] p-5 text-white md:p-6"><div class="flex items-center justify-between"><div><h3 class="text-xl font-bold">Catatan operasional</h3><p class="mt-1 text-xs text-[#a7b2ad]">Ringkasan dari kondisi database bengkel</p></div>`);
			_push(ssrRenderComponent(_component_Icon, {
				name: "lucide:clipboard-list",
				class: "h-5 w-5 text-[#e9b949]"
			}, null, _parent));
			...
			_push(`<!--]--></div></section><section class="grid grid-cols-1 gap-5 xl:grid-cols-[1.35fr_0.65fr]"><div class="border border-[#d9d8d1] bg-[#fbfaf6] p-5 md:p-6"><div class="flex items-center justify-between"><div><h3 class="text-base font-bold text-slate-900">Ringkasan servis</h3><p class="mt-1 text-xs text-slate-400">Aktivitas servis dalam 6 periode terakhir</p></div><button class="rounded-lg p-2 text-slate-400 hover:bg-slate-50">`);
			_push(ssrRenderComponent(_component_Icon, {
			...
			_push(`<!--]--></div></div><div class="bg-[#0f7771] p-6 text-white"><div class="flex items-center gap-2 text-[#e9b949]">`);
			_push(ssrRenderComponent(_component_Icon, {
				name: "lucide:activity",
				class: "h-4 w-4"
			}, null, _parent));
			_push(`<span class="text-xs font-bold uppercase tracking-wider">Operasional</span></div><h3 class="mt-6 text-2xl font-bold">Bengkel bergerak.</h3><p class="mt-2 text-sm leading-6 text-[#d4e2dc]">Jaga stok dan antrean supaya pekerjaan berikutnya tidak tertahan.</p><div class="mt-8 grid grid-cols-2 gap-3"><div class="border border-white/20 bg-white/10 p-3"><p class="text-3xl font-bold">${ssrInterpolate(unref(stats).services)}</p><p class="mt-1 text-[11px] text-[#d4e2dc]">Servis aktif</p></div><div class="border border-white/20 bg-white/10 p-3"><p class="text-3xl font-bold">${ssrInterpolate(unref(stats).lowStock)}</p><p class="mt-1 text-[11px] text-[#d4e2dc]">Perlu restock</p></div></div></div></section><section class="grid grid-cols-1 gap-5 xl:grid-cols-[1.35fr_0.65fr]"><div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_20px_rgb(15_23_42/0.03)]"><div class="flex items-center justify-between border-b border-slate-100 p-5 md:p-6"><div><h3 class="text-base font-bold text-slate-900">Servis terbaru</h3><p class="mt-1 text-xs text-slate-400">Aktivitas pekerjaan yang baru masuk</p></div>`);
			_push(ssrRenderComponent(_component_NuxtLink, {
				to: "/services",
...
//# sourceMappingURL=pages--YmrjVWq.mjs.map
