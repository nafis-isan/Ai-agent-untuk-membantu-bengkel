...
function TemplatePlugin(options, appConfig, componentDir) {
  const vue = {};
  const templates = getTemplates(options, appConfig.ui, void 0, void 0, vue);
    ...
    vite: {
      async config(config) {
        ...
        if (options.experimental?.componentDetection && componentDir) {
          const dirs = resolveExtraScanDirs(root, options.scanPackages, options.components ? options.components.dirs : void 0);
